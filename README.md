...
#### 2026-09-25 — Por qué el número no cambiaba: el archivo nunca llegaba (SW v144 / Horarios v15)
...
#### 2026-09-25 — La versión, sobre el logotipo — y mostrando la correcta (SW v143 / Horarios v14)
...
- **Archivos:** `js/version.js`, `js/version-horarios.js`, `index.html`, `index2.html`, `styles.css`, `sw.js`, `sw2.js`.

#### 2026-09-25 — La versión, junto a la marca (SW v142 / Horarios v13)
...
- Aparece en la ficha «Acerca de» junto al logotipo, y la chapita de abajo a la derecha se alimenta del mismo dato.
- **Horarios (`index2.html`) ahora también muestra su versión**, al pie del login junto a la marca. Antes no tenía ninguna.
- **Archivos:** `js/version.js` (nuevo), `js/version-horarios.js` (nuevo), `index.html`, `index2.html`, `sw.js`, `sw2.js`.
...
- **Mientras la función no esté desplegada**, la app lo dice con un aviso claro y la carga por texto funciona igual. Para activarla: `supabase secrets set ANTHROPIC_API_KEY=...` y `supabase functions deploy leer-planilla --no-verify-jwt`.
- **Fix (mismo día):** ese aviso no aparecía. Cuando la función no existe, Supabase responde 404 **sin cabeceras CORS**, el navegador bloquea la respuesta y `fetch` falla con `TypeError: Failed to fetch` — el chequeo de `r.status === 404` nunca llegaba a correr. La app mostraba el error rojo genérico *«No se pudo leer la foto… probá con más luz»*, que apuntaba al lado equivocado. Ahora el caso se detecta en el `catch` del propio `fetch` y cada situación tiene su mensaje: **no desplegada** (con los comandos), **sin conexión**, **401/403** (sugiere `--no-verify-jwt`) y error real de lectura. Los cinco caminos probados en navegador.
...
- Archivos: `js/conexiones-log.js` (nuevo), `js/mensajes-admin.js` (ítems + etiqueta + navegación), `js/app-init.js` (init), `index.html`. `supabase-config.js?v=56`, `app-init.js?v=40`, SW `fondo-admin-v49`, versión visible **v49**.
...
- Etiqueta de versión visible: **v41**. `supabase-config.js?v=48`, `app-init.js?v=38`, SW `fondo-admin-v41`.
