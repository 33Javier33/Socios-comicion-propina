...
const BASE_CONOCIMIENTO = [
    ...
    { id:'don1', cat:'anticipos', titulo:'¿Cómo funciona la sección Donaciones?', tags:['donacion','donaciones','colecta','aporte','ayuda','solidario','descuento'],
      resp:'Sirve para organizar una <strong>colecta entre socios</strong> cuando alguien necesita ayuda. Se escribe el motivo, se elige el área, se le pone un monto a cada socio que aporta y se registra. A cada donante se le <strong>descuenta su aporte del balance a recibir</strong> y lo ve en su app (propi.solicitada) con el motivo de la colecta.',
      pasos:['Entra a <strong>💝 Donaciones</strong>','Escribe el <strong>motivo</strong> — para quién es la ayuda. Ese texto es el que verá el socio y el que agrupa los aportes en una misma colecta','Elige el <strong>área</strong> para filtrar (o déjala en "Todas") y busca al socio si hace falta','Escribe el <strong>monto</strong> de cada socio que va a aportar. Si todos ponen lo mismo, usa "Mismo monto para todos" y presiona Aplicar','Revisa el resumen (cuántos socios y cuánto se junta) y presiona <strong>Registrar aportes</strong>','Los aportes quedan agrupados por colecta más abajo, con el total juntado'],
      nota:'💡 Lo recaudado NO se abona al balance del socio ayudado: la administración le entrega el dinero aparte, la app solo lleva la cuenta. Si te equivocas, el botón 🗑 de cada aporte lo anula y le devuelve el monto al socio.<br><br>🖨 <strong>Imprimir comprobante</strong> genera el documento con todos los que aportaron, ordenados por área y con firma. 💾 <strong>Guardar copia</strong> lo deja archivado en Documentación (HTML + planilla CSV) como respaldo del descuento — importante, porque <strong>Reiniciar Ausencias borra los aportes</strong>. Si intentas reiniciar con colectas sin copia, la app te avisa antes.' },
