...
async function canje_imprimir() {
    ...
    let fechaVis = ahora.toLocaleDateString('es-CL',{day:'2-digit',month:'2-digit',year:'numeric'});
    if (fechaInput) {
        const fp = fechaInput.split('-');
        if (fp.length === 3) fechaVis = `${fp[2]}/${fp[1]}/${fp[0]}`;
    }

    const horaHoy   = ahora.toLocaleTimeString('es-CL',{hour:'2-digit',minute:'2-digit'});
    ...
    // Folio único: CAN-AAAAMMDD-HHMMSS-INI
    const _resp = getSesionResponsableObj();
    const _pad  = n => String(n).padStart(2, '0');
    const folio = `CAN-${ahora.getFullYear()}${_pad(ahora.getMonth()+1)}${_pad(ahora.getDate())}-${_pad(ahora.getHours())}${_pad(ahora.getMinutes())}${_pad(ahora.getSeconds())}-${(_resp.ini||'SYS').replace(/[^A-Za-z0-9]/g,'')}`;
    ...
    const canjeSnapshot = {
        folio, nombre: nomInput, fecha: fechaVis, hora: horaHoy,
        responsable: firmaResp, correccion: firmaError || null,
        desglose, total
    };
